...
baselineFile="Jan11_100k.txt"

SMT_file1="Dec14-100k-A.txt"
SMT_file2="Dec14-100k-B.txt"
SMT_file3="Dec19-100k-A.txt"
SMT_file4="Dec19-100k-B.txt"

baselineOUT="baselineDup.csv"
allPairsOUT="allPairs1500.csv"
...
def main():
    ...
    execTimes=readFile(SMT_file3, SMT_file4, SMT=True)
    
    #only cover cjpeg_transupp onward---we've already covered the first three
    for i in range(3, len(benchmarkNames)):
        for j in range(i, len(benchmarkNames)):
            task1=benchmarkNames[i]
            task2=benchmarkNames[j]
            print(task1, task2, sep=',', end=',', file=open(allPairsOUT, "a"))
